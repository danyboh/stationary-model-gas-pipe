function [Cp_kg,Ro,Mm] = Cp_Vnic_func(p,T,x);
% Calculation of the isobaric
% by the equation VNIC SMV (GOST 30319.3).
% T = 240 - 480 K, P = 0.1 - 12 MPa
% Auxiliery files: DAT_VNIC, FvnicFunc, fdensfunc, DAT_CP0, CALKCP0.
% Format: [Cp_kg,Ro,Mm] = Cp_Vnic_func(p,T,x);
...
% Mm - molar mass, kg/kmol
 ...
 [Kvnic,z,A1,A2,A3,A4,Mm,Rom]=FvnicFunc(p,T,x);
 Ro = Mm * Rom ; % kg/m3
 Cvom=calkcpofunc(x,T); % kDg/(Kmol'*K)
 Cv=R*(Cvom/R+A3);
